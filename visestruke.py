...
def raspodelaVisestruke(pocetakDatum, krajDatum):
    scope = ["https://www.googleapis.com/auth/spreadsheets", "https://www.googleapis.com/auth/drive.file",
             "https://www.googleapis.com/auth/drive"]
    creds = ServiceAccountCredentials.from_json_keyfile_name("keys.json", scope)
    client = gspread.authorize(creds)

    python_test = client.open("VISESTRUKE I FONDANI").worksheet("VISESTRUKE I FONDANI")
    ...
    vrste_prva = [vrstaprva for vrstaprva in torat['VRSTA PRVA']]
    velicine_prva = [velicinaprva for velicinaprva in torat['VELICINA PRVA']]
    vrste_druga = [vrstadruga for vrstadruga in torat['VRSTA DRUGA']]
    velicine_druga = [velicinadruga for velicinadruga in torat['VELICINA DRUGA']]
    vrste_treca = [vrstatreca for vrstatreca in torat['VRSTA TRECA']]
    velicine_treca = [velicinatreca for velicinatreca in torat['VELICINA TRECA']]
    vrste_cetvrta = [vrstacetvrta for vrstacetvrta in torat['VRSTA CETVRTA']]
    velicine_cetvrta = [velicinacetvrta for velicinacetvrta in torat['VELICINA CETVRTA']]
    vrste_peta = [vrstapeta for vrstapeta in torat['VRSTA PETA']]
    velicine_peta = [velicinapeta for velicinapeta in torat['VELICINA PETA']]

    vrste = [vrste_prva, vrste_druga, vrste_treca, vrste_cetvrta, vrste_peta]
    vrste = [vrsta for list in vrste for vrsta in list]
    velicine = [velicine_prva, velicine_druga, velicine_treca, velicine_cetvrta, velicine_peta]
    velicine = [velicina for list in velicine for velicina in list]
    spajanje = []
    opis_bool = False
...
def lepljeniKartoniVisestruke(pocetakDatum, krajDatum):
    scope = ["https://www.googleapis.com/auth/spreadsheets", "https://www.googleapis.com/auth/drive.file",
             "https://www.googleapis.com/auth/drive"]
    creds = ServiceAccountCredentials.from_json_keyfile_name("keys.json", scope)
    client = gspread.authorize(creds)
